...
exports.list_all_states = function(req, res) {
    State.find({'device':req.params.deviceId}).sort('-created').limit(50).exec(function(err, states) {
        if (err)
            res.send(err);
...
exports.add_state = function(req, res) {
    ...
    Device.findOneAndUpdate({_id: req.params.deviceId}, {offline:false}, {new: true}, function(err, device) {
        if (err)
          res.send(err);
    });
    clearTimeout(devices[req.params.deviceId]);
    devices[req.params.deviceId] = setTimeout(function(){
        Device.findOneAndUpdate({_id: req.params.deviceId}, {offline:true}, {new: true}, function(err, device) {
            if (err)
              res.send(err);
            var new_log = new Log({device:req.params.deviceId,type:"error",msg:device.name+" is offline"});
